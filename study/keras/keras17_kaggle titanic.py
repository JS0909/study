...
train_set.replace({'Sex':{'male':0,'female':1}, 'Embarked':{'S':0,'C':1,'Q':2}}, inplace=True) # 성별을 남자면 0, 여자면 1로 대체 / Embarked에서 S면 0, C면 1, Q면 2로 모두 대체
...
log = model.fit(x_train, y_train, epochs=1, batch_size=32, callbacks=[Es], validation_split=0.2)
...
print(y_submit)
y_submit = y_submit.astype(int) # 이상하게 submission 파일이 1.0, 0.0으로 표기되서 int타입으로 전환시키는 작업(소수점 아예 없애는 작업)
